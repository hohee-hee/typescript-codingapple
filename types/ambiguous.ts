...
let a: number | string | boolean = 123; // 이 변수에는 num 또는 str이 들어올 수 있다.
a = true;
a = "kim";

// 여러가지 타입의 원소를 가진 array와 obj
let numbers: (number | string)[] = [1, 2, "kim"]; // 소괄호 유무에 따라 타입이 달라진다
let obj: { a: string | number } = { a: 123 };

// 2. any 타입 : 타입실드 해제 문법
// 하지만 이걸 쓰면 타입스크립트를 쓰는 이유가 없음
let b: any; // 아무거나 들어올 수 있다
b = 123;
b = "kim";
